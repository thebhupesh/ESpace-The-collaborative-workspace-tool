<?php
    ...
    $row=mysqli_fetch_array($res);
    $pid=$row[0];
    $sql="SELECT workspace_files.title,workspace_files.data FROM workspace_files WHERE WID ='$wid' AND workspace_files.version = '$version';";
    ...
    echo '<br><div class="toolbar">';
      ...
      echo '<div class="center">
        <div id="editor3" class="editor" contenteditable>
          <h1 id="T1">'.$title.'</h1>
          <p id="D1">'.$data.'</p>
        </div>
      </div>
      <div style="text-align: center">
        <form action="submitFile.php" method="post" name="workspace_form1">
          <input type="hidden" name="id" value="'.$id.'">
          <input type="hidden" name="wid" value="'.$wid.'">
          <input type="hidden" id="w_title1" name="title" value="">
          <input type="hidden" id="w_data1" name="data" value="">
        </form>
        <button class="w3-button w3-green w3-round" style="text-align: center" onclick="updateData1()" ><b>Submit</b></button>
      </div>
